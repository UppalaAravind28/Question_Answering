...
def main():
    ...
    # === About Section ===
    st.sidebar.header("ℹ️ About PulseBot")
    st.sidebar.markdown("""
    ...
    **Developed By**:  
    ...
    # === Custom CSS (F-string formatting) ===
    chat_styles = f"""
        <style>
        .chat-container {{
            max-height: 65vh;
    ...
    user_input = st.text_input(
        ...
        on_change=handle_user_input  # Call function to handle user input
    )

    # === Spinner only for the response logic ===
    if len(st.session_state.conversation_history) > 0 and not st.session_state.user_input:
